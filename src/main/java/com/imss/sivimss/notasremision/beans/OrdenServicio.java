...
public class OrdenServicio {
	...
	public DatosRequest listadoODS(BusquedaDto busqueda) throws UnsupportedEncodingException {
		...
		query.append("AND fin.ID_TIPO_ORDEN = 4");
	...
	public DatosRequest buscarODS(DatosRequest request, BusquedaDto busqueda, String formatoFecha)
		...
		if (busqueda.getIdDelegacion() != null && busqueda.getIdVelatorio() != null && busqueda.getFecIniODS() != null
				&& busqueda.getFecFinODS() != null) {
			query.append(" AND vel.ID_DELEGACION = ").append(busqueda.getIdDelegacion());
			query.append(" AND os.ID_VELATORIO = ").append(busqueda.getIdVelatorio());
			...
			return request;
		} else if (busqueda.getIdDelegacion() != null && busqueda.getIdVelatorio() == null
				&& busqueda.getFecIniODS() != null && busqueda.getFecFinODS() != null) {
			query.append(" AND vel.ID_DELEGACION = ").append(busqueda.getIdDelegacion());
			query.append(" AND DATE(nr.FEC_ALTA) BETWEEN STR_TO_DATE('" + busqueda.getFecIniODS() + "','" + formatoFecha
					+ "') AND STR_TO_DATE('" + busqueda.getFecFinODS() + "','" + formatoFecha + "')");
			busquedaGeneradas.append(query);
		...
		// Busqueda General
			...
			if (busqueda.getFecIniODS() != null) {
				query.append(
						" AND DATE(nr.FEC_ALTA) BETWEEN STR_TO_DATE('" + busqueda.getFecIniODS() + "','" + formatoFecha
								+ "') AND STR_TO_DATE('" + busqueda.getFecFinODS() + "','" + formatoFecha + "')");
			}
	...
	private StringBuilder busquedaCancelada(String formatoFecha) {
		StringBuilder query = new StringBuilder(
				"SELECT DISTINCT os.ID_ORDEN_SERVICIO AS id, os.CVE_FOLIO AS folioODS, DATE_FORMAT(os.FEC_ALTA,'"
						+ formatoFecha
						+ "') AS fechaODS, IFNULL(cvn.DES_FOLIO,0) AS folioConvenio, os.ID_CONTRATANTE AS idContratante, ");
		query.append(
				"CONCAT(IFNULL(prc.NOM_PERSONA,' '),IFNULL(prc.NOM_PRIMER_APELLIDO,' '),IFNULL(prc.NOM_SEGUNDO_APELLIDO,' ')) AS nomContratante, \n");
		...
		query.append(
				"JOIN SVT_NOTA_REMISION nr ON (os.ID_ORDEN_SERVICIO = nr.ID_ORDEN_SERVICIO) AND nr.ID_ESTATUS =3 \n");
		query.append("JOIN SVC_VELATORIO vel ON (vel.ID_VELATORIO = os.ID_VELATORIO) \n");
	...
	private StringBuilder busquedaGeneradas(String formatoFecha) {
		...
		query.append("IFNULL(nr.ID_ESTATUS,1) AS estatus, IFNULL(nr.ID_NOTAREMISION,0) AS idNota,  \n");
		query.append(
				"IFNULL(nr.ID_NOTAREMISION,0) AS idCancelada, (SELECT COUNT(rn.ID_ORDEN_SERVICIO) FROM SVT_NOTA_REMISION rn WHERE rn.ID_ORDEN_SERVICIO=os.ID_ORDEN_SERVICIO ) AS total \n");
		query.append("FROM SVC_ORDEN_SERVICIO os \n");
		query.append("JOIN SVC_CONTRATANTE con ON (os.ID_CONTRATANTE = con.ID_CONTRATANTE) \n");
		query.append("JOIN SVC_PERSONA prc ON (con.ID_PERSONA = prc.ID_PERSONA) \n");
		query.append("JOIN SVT_CONTRATANTE_PAQUETE_CONVENIO_PF cpcf ON (con.ID_CONTRATANTE = cpcf.ID_CONTRATANTE) \n");
